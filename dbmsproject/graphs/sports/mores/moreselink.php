...
$data = array();
$q1= "select SUBSTRING(link,8,15) as `slink` , count(*) as `count`  from sportse where id not in(select id from sportsp  where headline like '%tennis%' or link like '%tennis%' or headline like '%football%' or link like '%football%'  or headline like '%cricket%' or link like '%cricket%') group by slink;";
